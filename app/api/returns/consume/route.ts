// app/api/returns/consume/route.ts

import { NextResponse } from "next/server";
import crypto from "crypto";

const EDGE_FN = "receipt-consume";
...
export async function POST(req: Request) {
  try {
    ...
    if (!verifierKey) {
      return NextResponse.json({ ok: false, error: "missing_verifier_key", request_id: crypto.randomUUID() }, { status: 401 });
    }

    const bodyText = await req.text();
    const bodyBytes = Buffer.from(bodyText, "utf8");

    // IMPORTANT: edge canonicalizes to "/receipt-consume" (function name only)
    const path = `/${EDGE_FN}`;
    ...
    return NextResponse.json(
      { ok: false, error: "proxy_unhandled_error", details: msg, request_id: crypto.randomUUID() },
      { status: 500 }
    );
  }
}
